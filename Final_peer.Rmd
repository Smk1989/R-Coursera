...
load("~/R Coursera/ames_train.Rdata")
```
...
```{r}
initial_model<-lm(formula = log(price) ~ area + Neighborhood + Overall.Qual + 
    Overall.Cond + Year.Built + Bedroom.AbvGr, 
    data = ames_train)
model_sum<-summary(initial_model)
model_sum$adj.r.squared
model_sum
```

Using BAS modelling technique
...
rm("summary2")
rm("calc_quality")
initial_model_bas<-bas.lm(log(price) ~ area + Neighborhood + Overall.Qual+Overall.Cond + Year.Built + Bedroom.AbvGr + TotRms.AbvGrd,data=ames_train,prior="BIC",modelprior = uniform())
